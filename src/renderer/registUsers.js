...
btnAgregar.addEventListener("click", (e) => {
  const regexIdUsuario = new RegExp("^([a-zA-Z]{0}[0-9]{6,10}[a-zA-Z]{0})$");
  const regexNombreUsuario = new RegExp("^([a-zA-Z]+)$");
  const regexApellidoUsuario = new RegExp("^([a-zA-Z]+)$");
  const regexTelefonoUsuario = new RegExp(
    "^([a-zA-Z]{0}[0-9]{10}[a-zA-Z]{0})$"
  );
  ...
  if (
    ...
    if (regexIdUsuario.test(idUsuario.value) && regexNombreUsuario.test(nombreUsuario.value.replace(/\s/g, "")) && regexApellidoUsuario.test(apellidoUsuario.value.replace(/\s/g, "")) && regexTelefonoUsuario.test(telefonoUsuario.value)) {
      const user = {
      ...
      if (regexNombreUsuario.test(apellidoUsuario.value.replace(/\s/g, ""))) {
        validateFirstName.innerHTML += `
        <small>Solo se permiten letras.</small>`;
      }
      if (regexApellidoUsuario.test(apellidoUsuario.value.replace(/\s/g, ""))) {
        validateLastName.innerHTML += `
        <small>Solo se permiten letras.</small>`;
      }
